...
object Day05 extends App {
  ...
  val sampleOutput = IntComputer.runComputer(IntComputerProgress(sampleInput, List(9)))
  println(s"Sample output: ${sampleOutput.output}")

  val output = IntComputer.runComputer(IntComputerProgress(sourceCode, List(1)))
  println(s"Output: ${output.output}")

  val outputTwo = IntComputer.runComputer(IntComputerProgress(sourceCode, List(5)))
